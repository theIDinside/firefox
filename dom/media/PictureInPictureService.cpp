...
RefPtr<Promise> PictureInPictureService::AssociatePictureInPictureWindowWith(
  ...
  AutoJSAPI jsapi;
  if (NS_WARN_IF(!jsapi.Init(aElement->GetOwnerGlobal()))) {
    return nullptr;
  }
...
bool PictureInPictureService::InitializeFunctions() {
  if (!mPictureInPictureProvider) {
    mPictureInPictureProvider =
        do_CreateInstance("@mozilla.org/toolkit/pictureinpicture-provider");
...
void EnterPictureInPictureRequest::OnServicePromiseSettled(bool aResolved) {
  ...
  if (aResolved && mPromise) {
    ...
    // to exit fullscreen.
    if (mVideo == doc->GetFullscreenElement()) {
