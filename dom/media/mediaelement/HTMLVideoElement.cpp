...
void HTMLVideoElement::ClosePictureInPictureWindowAndFireEvent() {
  ...
  // If this is GetPictureInPictureElementInternal, then
  // we cloned this video using PIP API, not our native feature.
...
already_AddRefed<Promise> HTMLVideoElement::RequestPictureInPicture(
  ...
  RefPtr<Promise> p = Promise::Create(GetOwnerGlobal(), aRv);
  if (!p) {
    return nullptr;
  }
  ...
  if (!doc->PictureInPictureEnabled()) {
    p->MaybeRejectWithSecurityError("Not allowed to use Picture-in-Picture");
    return p.forget();
  }
  ...
  // 4. If this has no video track, reject p with an InvalidStateError exception
  // and return p.
  if (!HasVideo()) {
  ...
  // 6.1. If this’s relevant global object does not have a transient activation
  // reject p with a NotAllowedError exception and return p.
  // 6.2. Consume user activation given this’s relevant global object.
  Element* pictureInPictureElement = doc->GetPictureInPictureElement();
